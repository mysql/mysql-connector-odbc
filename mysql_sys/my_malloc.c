...
static void *my_raw_malloc(size_t size, myf my_flags)
{
  ...
  if (point == NULL)
  {
    set_my_errno(errno);
    if (my_flags & MY_FAE)
      sys_error_handler_hook=fatal_sys_error_handler_hook;
    if (my_flags & (MY_FAE+MY_WME))
      mysys_error(EE_OUTOFMEMORY, MYF(ME_ERRORLOG + ME_FATALERROR),size);
    DBUG_EXECUTE_IF("simulate_out_of_memory",
...
static void *my_raw_realloc(void *oldpoint, size_t size, myf my_flags)
{
  ...
  if (point == NULL)
  {
    ...
    set_my_errno(errno);
    if (my_flags & (MY_FAE+MY_WME))
      mysys_error(EE_OUTOFMEMORY, MYF(ME_FATALERROR),
               size);
    DBUG_EXECUTE_IF("simulate_out_of_memory",
                    DBUG_SET("-d,simulate_out_of_memory"););
  }
