...
#define MY_INIT(name)   { mysys_progname= name; my_init(); }
...
extern char *sys_home_dir;			/* Home directory for user */
extern const char *mysys_progname;		/* program-name (printed in errors) */
extern char sys_curr_dir[];		/* Current directory for user */
extern void (*sys_error_handler_hook)(uint my_err, const char *str,myf MyFlags);
extern void (*fatal_sys_error_handler_hook)(uint my_err, const char *str,
				       myf MyFlags);
extern void (*sys_local_message_hook)(enum loglevel ll,
                                  const char *format, va_list args);
extern uint mysys_file_limit;
extern MYSQL_PLUGIN_IMPORT ulong mysys_thread_stack_size;
...
*/
extern void (*enter_sys_cond_hook)(void *opaque_thd,
                               mysql_cond_t *cond,
                               mysql_mutex_t *mutex,
...
extern void (*exit_sys_cond_hook)(void *opaque_thd,
                              const PSI_stage_info *stage,
                              const char *src_function,
                              const char *src_file,
                              int src_line);
...
*/
extern int (*is_sys_killed_hook)(const void *opaque_thd);
...
/* statistics */
extern ulong	mysys_file_opened,mysys_stream_opened, mysys_tmp_file_created;
extern ulong    mysys_file_total_opened;
extern my_bool	mysys_init_done;

extern MYSQL_PLUGIN_IMPORT int mysys_umask;		/* Default creation mask  */
extern int mysys_umask_dir;

extern ulong	mysys_default_record_cache_size;
extern my_bool  mysys_disable_locking,
                mysys_enable_symlinks;
...
extern struct st_my_file_info *mysys_file_info;
...
extern HANDLE   mysys_get_osfhandle(File fd);
...
extern char *mysys_strerror(char *buf, size_t len, int errnum);
extern const char *mysys_get_err_msg(int nr);
extern void mysys_error(int nr,myf MyFlags, ...);
extern void mysys_printf_error(uint my_err, const char *format,
                            myf MyFlags, ...)
  MY_ATTRIBUTE((format(printf, 2, 4)));
extern void mysys_printv_error(uint error, const char *format, myf MyFlags,
                            va_list ap);
extern int mysys_error_register(const char* (*get_errmsg) (int),
                             int first, int last);
extern my_bool mysys_error_unregister(int first, int last);
extern void mysys_message(uint my_err, const char *str,myf MyFlags);
extern void mysys_message_stderr(uint my_err, const char *str, myf MyFlags);
void mysys_message_local_stderr(enum loglevel ll,
                             const char *format, va_list args);
extern void mysys_message_local(enum loglevel ll, const char *format, ...);
...
extern int sys_test_if_hard_path(const char *dir_name);
extern my_bool sys_has_path(const char *name);
...
#ifdef _WIN32
extern my_bool sys_have_tcpip;		/* Is set if tcpip is used */
...
void mysys_init_mysys_psi_keys(void);
#endif

struct st_mysql_file;
extern struct st_mysql_file *mysys_stdin;
